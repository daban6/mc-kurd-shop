...
export default function ShopPage() {
  ...
  // ── Fetch products whenever API-level filters change
  useEffect(() => {
    let cancelled = false;
    ...
    async function load() {
      ...
      try {
        ...
        const sp = SORT_PARAM[sortBy];
        if (sp) qs.set("sort", sp);
  ...
  // ── Client-side price filter (applied after API fetch)
  const filteredProducts = products.filter((p) => {
    const matchesMin = !minPrice || p.priceIqd >= parseInt(minPrice);
    const matchesMax = !maxPrice || p.priceIqd <= parseInt(maxPrice);
    return matchesMin && matchesMax;
  });

  // ── Sidebar category list (static "All" + API)
  const sidebarCategories = [
    { slug: "all", nameEn: "All", nameKu: "هەموو", icon: Package },
    ...apiCategories.map((cat) => ({
      slug:   cat.slug,
      nameEn: cat.nameEn,
      nameKu: cat.nameKu,
      icon:   CATEGORY_ICONS[cat.slug] ?? Package,
    })),
  ];
  ...
  if (selectedCategory !== "all") {
    ...
    if (cat) activeFilters.push({ key: "category", label: isKurdish ? cat.nameKu || cat.nameEn : cat.nameEn });
  }
  if (minPrice) activeFilters.push({ key: "minPrice", label: `Min: ${formatPrice(parseInt(minPrice))}` });
  if (maxPrice) activeFilters.push({ key: "maxPrice", label: `Max: ${formatPrice(parseInt(maxPrice))}` });

  const clearFilter = (key: string) => {
    if (key === "category") setSelectedCategory("all");
    if (key === "minPrice") { setMinPrice(""); setCurrentPage(1); }
    if (key === "maxPrice") { setMaxPrice(""); setCurrentPage(1); }
  };
  ...
  // ── Filter sidebar (defined here to close over state)
  const FilterSidebar = () => (
        ...
        <div className="flex flex-col gap-1">
          {sidebarCategories.map((cat) => (
            ...
            >
              ...
              {isKurdish ? cat.nameKu : cat.nameEn}
            </button>
          ))}
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 max-h-[80vh] overflow-y-auto rounded-t-xl bg-background p-5">
              <div className="mb-4 flex items-center justify-between">
                ...
                <button onClick={() => setShowMobileFilters(false)} className="text-muted hover:text-foreground">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <FilterSidebar />
        ...
        <main className="min-w-0 flex-1 lg:pl-5">
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            <span className="text-sm text-muted">
              {filteredProducts.length} {isKurdish ? "بەرهەم" : "products"}
            </span>
            {activeFilters.length > 0 && (
              <>
                ...
                {activeFilters.map((filter) => (
                  ...
                  >
                    {filter.label}
                    <button onClick={() => clearFilter(filter.key)} className="hover:text-foreground">
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                ))}
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="mt-3 text-sm text-muted">
                {isKurdish ? "هیچ بەرهەمێک نەدۆزرایەوە" : "No products found"}
              </p>
              <button
                onClick={clearAllFilters}
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                {paginatedProducts.map((product) => {
                  ...
                  const categoryName = isKurdish
                    ? product.category?.nameKu || product.category?.nameEn
